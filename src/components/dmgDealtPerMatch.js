...
class DmgDealtPerMatch extends Component {

    constructor(){
        super();
        this.state = {
            greaterDamageDealtPerMatch: []   
        }
    }

    componentDidMount(){
        fetch('http://localhost:8080/rankedgg/greaterDamagePerMatch/Papay')
        .then(results => {
          return results.json();
        }).then(data => {
          this.setState({greaterDamageDealtPerMatch: data});
        })
    }
        ...
        return (
            <div className="x_content">
                <table className="ui celled table" id="crititalStrikes">
                    ...
                    <tbody>
                        {this.state.greaterDamageDealtPerMatch.map(data => {
...
export default DmgDealtPerMatch;