...
class FasterMatchs extends Component {
    ...
    componentDidMount(){
        fetch('http://localhost:8080/rankedgg/fasterMatchs/Papay')
        .then(results => {
          return results.json();
        }).then(data => {
          this.setState({fasterMatchs: data});
        })
    }
        ...
        return (
            <div className="x_content">
                <table className="ui celled table" id="crititalStrikes">
                    ...
                    <tbody>
                        {this.state.fasterMatchs.map(data => {
